 ...
 * AppBar expands or shrinks according to the isExpanded status.
 * @param {Number} [expandOffset=0] - Expand offset width in pixel.
      ...
      <MuiAppBar
        classes={{
          root: classNames(classes.appBar, {
            [classes.appBarShrink]: !isExpanded,
          }),
        }}
        style={{
          width: isExpanded === true ? '100%' : `calc(100% - ${expandOffset}px)`,
        }}
        color='inherit'
      >
        <Toolbar>
          <IconButton onClick={this.handleExpandToggle.bind(this)}>
